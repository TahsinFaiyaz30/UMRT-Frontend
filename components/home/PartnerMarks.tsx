...
import { useSection } from './SectionShell';
import styles from './HomeSections.module.css';

export function PartnerMarks() {
  const { records } = useContentRecords('partners');
  const section = useSection('partner-marks');
  if (records.length === 0) return null;

  return (
    <section
      className={`${styles.section} ${styles.partners}`}
      aria-label="Institutional backing"
    >
      <div className={styles.inner}>
        <div className={`${styles.partnerShell} ${styles.panel}`}>
          <p className={styles.partnersKicker}>
            <i className={styles.signalDot} aria-hidden="true" />
            {section?.kicker ?? 'Support network'}
          </p>
